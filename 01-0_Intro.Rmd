...
Posit the company used to be called RStudio. I thought that RStudio, the software, was also going to change names, because it is not just for working with R, but it hasn't yet. Posit now makes RStudio (which is still open source!) and also hosts RStudio servers via Posit.cloud that we pay for to help you all learn R, basic programming, data science, and engineering data analytics. 
...
This provides a concise introduction to markdown and it's power. Through simple text symbols your can format text to be headings (with #'s), make text **bold**, denote what is `code`, and offset code chunks with three backticks (the key next to 1) and curly braces indicating the programming language and maybe even naming or changing some settings with how the code should be run and output.  

**Note how the above and all of this document has empty lines between text elements. This makes sure that particularly headings and sections, like numbered/bullet lists, are formatted correctly.**
...
There are some things the visual editor does make easier like inserting images and citations and formatting tables.
...
In general this works quite well. 
...
### Code chunk settings and global settings
